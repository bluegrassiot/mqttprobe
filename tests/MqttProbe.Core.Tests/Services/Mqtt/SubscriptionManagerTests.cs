...
public class SubscriptionManagerTests
{
    ...
    public void Setup()
    {
        ...
        _manager = new SubscriptionManager(_mockClient, _mockLogger, _notifier, _mockConnectionSettings, _mockUiSettings, _mockSessionState);
    }
    ...
    public async Task Add_DuplicateTopic_DoesNotResubscribe()
    {
        ...
        ((FakeUserNotifier)_notifier).Notifications.Should().Contain(n =>
            n.Severity == UserNotificationSeverity.Warning && n.Message.Contains("Already subscribed", StringComparison.OrdinalIgnoreCase));
    }
    ...
    [Test]
    public async Task OnSyncFailed_ShowsErrorNotification()
    {
        ...
        ((FakeUserNotifier)_notifier).Notifications.Should().Contain(n =>
            n.Severity == UserNotificationSeverity.Error);
    }
    ...
    public async Task Add_AtMaxSubscriptions_RejectsNewTopic()
    {
        ...
        ((FakeUserNotifier)_notifier).Notifications.Should().Contain(n =>
            n.Severity == UserNotificationSeverity.Warning && n.Message.Contains("limit", StringComparison.OrdinalIgnoreCase));
        await _mockClient.DidNotReceive().SubscribeAsync(Arg.Any<IEnumerable<MQTTnet.Packets.MqttTopicFilter>>());
    }
    ...
    public async Task Add_InvalidTopic_NullChar_ShowsWarning()
    {
        ...
        await _mockClient.DidNotReceive().SubscribeAsync(Arg.Any<IEnumerable<MQTTnet.Packets.MqttTopicFilter>>());
        ((FakeUserNotifier)_notifier).Notifications.Should().Contain(n =>
            n.Severity == UserNotificationSeverity.Warning);
    }
